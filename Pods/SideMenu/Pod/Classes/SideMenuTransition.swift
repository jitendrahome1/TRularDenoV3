...
open class SideMenuTransition: UIPercentDrivenInteractiveTransition {
    ...
    public static let singleton = SideMenuTransition()
    public static var presentDirection: UIRectEdge = .left
    public static weak var tapView: UIView? {
        didSet {
    ...
    public class func handleHideMenuPan(_ pan: UIPanGestureRecognizer) {
        if activeGesture == nil {
            activeGesture = pan
        } else if pan != activeGesture {
            pan.isEnabled = false
    ...
    public class func handleHideMenuTap(_ tap: UITapGestureRecognizer) {
        presentingViewControllerForMenu?.dismiss(animated: true, completion: nil)
    }
    
    public class func hideMenuStart() {
    ...
    public class func hideMenuComplete() {
        guard let mainViewController = presentingViewControllerForMenu,
